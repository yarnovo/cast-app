...
/**
 * e2e · cast-app 单页对话 · 多轮 session 真续上 (REQ-003)
 *
 * 验证:
 *   1. 进 / · 直接看到 chat-header (meta 头像 + 名 + 重开 + 用户头像)
 *   2. session_id 自动生成 · 落 sessionStorage(`cast_session`) + url query (?s=)
 *   3. 同 session 连发 2 条 · cast-agents 真续上 (暗号 turn 2 验)
 *   4. 重开按钮: session_id 换新 + history 清 + INTRO 重出
 *   5. 跨刷新 session 不丢
 *   6. 没其他页 (整个 app 单页 · 没 BottomNav)
 *
 * 网络: page.route 拦截 + APIRequestContext server-to-server 真调 prod cast-agents
 *      (CORS 允许 m.cast.agentaily.com origin · 本地 localhost:4173 必须走拦截)
 */
...
test('cast-app 单页 · 多轮真续上 + 重开换 sid', async ({ page }) => {
  ...
  // 1. 进 / · 直接渲 ChatPage (单页 · 没 BottomNav 没 list)
  await page.goto('/')
  ...
  // 5. 第 2 轮 (同 session): 问暗号 · LLM 应能从上一轮 history 拿到
  ...
  // 6. 验 2 轮请求都用同一个 session_id (server-side 真续)
